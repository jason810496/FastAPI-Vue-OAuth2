...
@router.get("/{username}" , response_model=user_schema.Base)
async def get_user_by_username(username : str , db: UserCRUD = Depends(get_user_crud)):
    db_user = await db.get_user_by_username(username=username)
    if db_user is None:
